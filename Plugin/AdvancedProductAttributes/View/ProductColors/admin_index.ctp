...
<div id="column_right">
	<div id="action_top">
		...
		<?php echo $this->Form->create('ProductColor',array('url'=>array('action'=>'index'),'type'=>'get','name'=>'search','inputDefaults'=>array('div'=>false,'label'=>false)))?>
		<ul class="search">
			...
			<li class="counter"><?php echo __('Tìm thấy',true).' '.$counter_c.' '.__('Màu sắc',true)?></li>
		</ul> <!--  end .search_name -->
	...
	<?php
		echo $this->Form->create('ProductColor',array('type'=>'post','name'=>'form','inputDefaults'=>array('div'=>false,'label'=>false)));
	...
	<div id="content">
		<table class="list">
			<tr class="first">
				...
				<th><?php echo $this->Paginator->sort('color',__('Tên',true))?></th>
				<th><?php echo __('Mã màu (hex)',true)?></th>
			...
			<?php foreach($product_colors as $val){
				$item_product_color = $val['ProductColor'];

				$url_edit = array('controller'=>'product_colors','action'=>'edit',$item_product_color['id'],'?'=>array('url'=>$current_url_c));
			?>
			<tr id="<?php echo 'item_'.$item_product_color['id']?>">
				...
				<td>
					<?php echo $item_product_color['color'];
					?>
				</td>
				<td class="center" style="background: <?php echo $item_product_color['hex'] ?>"><?php echo $item_product_color['hex'] ?></td>

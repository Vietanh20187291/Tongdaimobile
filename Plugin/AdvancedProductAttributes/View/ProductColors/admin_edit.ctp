...
<div id="column_right">
	...
	<div id="content">
		<?php
			echo $this->Form->create('ProductColor',array('id'=>'form','url'=>array('plugin'=>'AdvancedProductAttributes','action'=>'edit','?'=>array('url'=>(!empty($_GET['url']))?urldecode($_GET['url']):'')),'inputDefaults'=>array('label'=>false,'div'=>false)));
		...
		<div class="tab_container">
			...
			<div id="tab1" class="tab_content">
				<table class="add">
					<tr>
						<th><?php echo $this->Form->label('color',__('Tên màu',true))?> <span class="im">*</span></th>
						<td><?php echo $this->Form->input('color',array('class'=>'larger'))?></td>
					</tr>

					<tr>
						<th><?php echo $this->Form->label('hex',__('Mã màu',true))?> <span class="im">*</span></th>
						<td><?php echo $this->Form->input('hex',array('class'=>'w52', 'style'=>'background-color: '.$this->request->data['ProductColor']['hex']))?></td>
					</tr>
...
$('#ProductColorHex').ColorPicker({
	...
	onChange: function (hsb, hex, rgb) {
		$('#ProductColorHex').val('#' + hex);
		$('#ProductColorHex').css('background-color', '#' + hex);
	}
})
.bind('keyup', function(){
	$(this).ColorPickerSetColor(this.value);

<aside class="box product_run<?php if(!empty($class)) echo ' '.$class ?>">
	...
	<ul id="show_<?php echo $position ?>" class="show nav">
		...
		foreach($data as $val){
			...
			$url = array('plugin'=>false,'controller'=>'products','action'=>'index','lang'=>$item_product['lang']);
		...
		<li>
			<div class="thumb">
				<?php 
					...
					echo $this->Html->link($this->HtmlAmp->thumbAmp('products/'.$item_product['image'],array('alt'=>$item_product['meta_title'],'width'=>$w,'height'=>$h)),$url,$link_img_attr);
					
					if(!empty($oneweb_product['cart_button']) && !empty($oneweb_product['order'])){
				?>
				<p class="cart"><?php echo $this->Html->link(__('Mua',true),'#',array('title'=>__('Mua',true),'rel'=>'nofollow'))?></p>
				<?php }?>
			</div> <!--  end .thumb -->

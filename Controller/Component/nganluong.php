...
class NganluongComponent{
	// Địa chỉ thanh toán hoá đơn của NgânLượng.vn
	private $nganluong_url = 'https://www.nganluong.vn/checkout.php';			// Dùng đường dẫn này để checkout khi web đi vào hoạt động
//	private $nganluong_url = 'http://sandbox.nganluong.vn/checkout.php';		// Dừng đường dẫn này để checkout khi testing		
	
	
	
	// Mã website của bạn đăng ký trong chức năng tích hợp thanh toán của NgânLượng.vn.
	private $merchant_site_code = '22985'; //Dùng mã này khi web đi vào hoạt động (Tùy website sẽ có mã khác nhau)
//	private $merchant_site_code = '275';	// Dùng mã này để test
	
	// Mật khẩu giao tiếp giữa website của bạn và NgânLượng.vn.
	private $secure_pass= 'urL193'; //Dùng pass này khi web đã đi vào hoạt động
//	private $secure_pass= '123456';			// Dùng pass này để test
...
//		return $redirect_url.$url;
	...
	function RedirectToNganluong($url_nganluong){
//		debug($url_nganluong); die();
//		$this->redirect($url_nganluong);
		$payPalURL = "http://sandbox.nganluong.vn/checkout.php?merchant_site_code=275&return_url=http%3a%2f%2flocalhost%2fbinh%2fpayments%2fresult_nganluong&receiver=kieunghia.luckystar%40gmail.com&transaction_info=give+the+Stamping-Press+Operator+contacts&order_code=10-8491&price=15000&secure_code=c99d2f5de7c130cfd1b42269f61f8fcd";
//		$payPalURL = "https://www.sandbox.paypal.com/webscr?cmd=_ap-payment&paykey=AP-4SW6897858347300H"; 
//        debug($payPalURL); die();
        header("Location: " . $payPalURL);
//		header("Location: " . $url_nganluong);
	}

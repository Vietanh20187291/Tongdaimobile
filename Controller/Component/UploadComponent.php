...
//		}
		...
		/**
		  ...
		  *			- rules['type'] = string ('resize','resizemin','resizemax','resizecrop','crop')
...
//			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','JPG','jpeg','JPEG','gif','GIF','png','PNG'); }
			if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','JPG','jpeg','JPEG','gif','GIF','png','PNG','swf','SWF','doc','DOC','docx','DOCX','pdf','PDF','zip','ZIP','rar','RAR','xls','XLS','xlsx','XLSX','ppt','PPT','pptx','PPTX','txt','TXT','dwg','DWG','bak','BAK','dxf','DXF'); }
		...
		function image ($file, $type, $size, $output = NULL, $quality = NULL) {
			...
			switch ($type) {
				...
				case 'resizemax':
					/**************  
					 *  author: HTA
					 *  Resize theo kich thuoc lon hon
					 * **************/
					...
					if($uploadWidth<=$maxW && $uploadHeight<=$maxH){
						$newX = $uploadWidth;
						$newY = $uploadHeight;
					}elseif (($uploadWidth == $maxW) && ($uploadHeight == $maxH)) {
						$newX = $uploadWidth;
						$newY = $uploadHeight;
					} else if (($ratioX * $uploadHeight) < $maxH) {
						$newX = $maxW;

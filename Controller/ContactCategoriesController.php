...
class ContactCategoriesController extends AppController {
	...
	public function admin_index() {
			...
			switch ($_POST['action']){
				case 'active':
					foreach ($_POST['chkid'] as $val){
						$this->ContactCategory->id = $val;
						$this->ContactCategory->set(array('status'=>1));
						$this->ContactCategory->save();
					}
				...
				case 'unactive':
					foreach ($_POST['chkid'] as $val){
						$this->ContactCategory->id = $val;
						$this->ContactCategory->set(array('status'=>0));
						$this->ContactCategory->save();
					}
				...
				case 'delete':
					foreach ($_POST['chkid'] as $val){
						$this->ContactCategory->delete($val);
					}
		...
		$this->ContactCategory->bindModel(array(
			'hasMany'=>array(
				'Contact' => array(
								'className' => 'Contact',
		...
		$this->ContactCategory->recursive = 1;
		$this->paginate = array(
		...
		$counter = $this->ContactCategory->find('count',array('recursive'=>-1));
		...
		$a_list_categories = $this->ContactCategory->find('all',array('order'=>'sort asc'));
	...
	function admin_ajaxChangeSort(){
		...
		$this->ContactCategory->id = $_POST['id'];
		$this->ContactCategory->set(array('sort'=>$_POST['val']));
		$this->ContactCategory->save();
	}
	...
	public function admin_ajaxDeleteItem() {
		...
		if($this->ContactCategory->delete($_POST['id'])) return true;
		else return false;
	}
	...
	public function admin_add() {
		if ($this->request->is('post')) {
			$data = $this->request->data['ContactCategory'];
			//Sắp xếp
			if(empty($data['sort'])) $data['sort'] = $this->ContactCategory->find('count',array('recursive'=>-1))+1;
			
			//Ngay tao
			$data['created'] = mktime(date('H'),date('i'),date('s'),date('m'),date('d'),date('Y'));
			
			$this->ContactCategory->create();
			if ($this->ContactCategory->save($data)) {
				$id = $this->ContactCategory->getLastInsertID();
	...
	public function admin_edit($id = null) {
		...
		if (!$this->ContactCategory->exists()) throw new NotFoundException(__('Invalid'));
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if(empty($data['sort'])){
				$this->ContactCategory->recursive = -1;
				$a_category = $this->ContactCategory->read('sort',$id);
			...
			$this->request->data = $this->ContactCategory->read(null, $id);
		}
	}
}

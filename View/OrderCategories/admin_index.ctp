...
<div id="column_right">
	<div id="action_top">
		...
		<?php echo $this->Form->create('Order',array('type'=>'get','name'=>'search','url'=>array('controller'=>'orders','action'=>'index'),'inputDefaults'=>array('div'=>false,'label'=>false)))?>
		<ul class="search">
			...
			<li><?php echo $this->Form->input('keyword',array('value'=>(!empty($_GET['keyword']))?$_GET['keyword']:__('Mã giao dịch, họ tên, email, phone',true),'class'=>'larger','onblur'=>'if (this.value==""){ this.value="'.__('Mã giao dịch, họ tên, email, phone',true).'";}','onfocus'=>'if (this.value=="'.__('Mã giao dịch, họ tên, email, phone',true).'") { this.value=""; }'))?></li>
	...
	<?php 
		echo $this->Form->create('OrderCategory',array('type'=>'post','name'=>'form','inputDefaults'=>array('div'=>false,'label'=>false)));
	...
	<div id="content">
		<table class="list">
			...
			<?php foreach($a_categories_c as $val){
				...
				$url_edit = array('controller'=>'order_categories','action'=>'edit',$item_category['id'],'?'=>array('url'=>$current_url_c));
			?>
			<tr id="<?php echo 'item_'.$item_category['id']?>">
				<td class="center"><input type="checkbox" name="chkid[]" value="<?php echo $item_category['id']?>" /></td>
				<td><?php echo $this->Html->link($this->Text->truncate($item_category['name'].' ('.count($val['Order']).')',200,array('extact'=>false)),array('controller'=>'orders','action'=>'index','?'=>array('category_id'=>$item_category['id'])),array('title'=>$item_category['name'],'class'=>'tooltip'));?></td>

...
<div id="column_right">
	...
	<div id="content">
		<?php 
			echo $this->Form->create('ProductMaker',array('type'=>'file','id'=>'form','url'=>array('action'=>'edit','?'=>array('url'=>(!empty($_GET['url']))?urldecode($_GET['url']):'')),'inputDefaults'=>array('label'=>false,'div'=>false)));
			...
			<div id="tab1" class="tab_content">
				<table class="add column1">
					...
					<tr>
						...
						<td id="image">
							<?php 
								if(!empty($this->request->data['ProductMaker']['image'])){
									if(!empty($this->request->data['ProductMaker']['image'])) $img = '/timthumb.php?src='.$this->Html->url('/webroot/img/images/product_makers/').$this->request->data['ProductMaker']['image'];
									$img_small = $img."&h=50&w=50&zc=2";
									$img_larger = $img."&h=100&w=100&zc=2";
									
									echo $this->Html->link($this->Html->image($img_small,array('alt'=>$this->request->data['ProductMaker']['name'])),$img_larger,array('title'=>$this->request->data['ProductMaker']['name'],'class'=>'preview','target'=>'_blank','escape'=>false));
									echo $this->Html->link('&nbsp;',"javascript:delImg({$this->request->data['ProductMaker']['id']},'image');",array('title'=>'Xóa ảnh','class'=>'act delete tooltip ask','escape'=>false)).'<br />';
								}
								echo $this->Form->input('image',array('type'=>'file'));
								echo $this->Html->tag('span',"Kích thước: {$oneweb_product['size']['maker'][0]} x {$oneweb_product['size']['maker'][1]} (px)",array('class'=>'size_img'));
							?>
						</td>
					</tr>
					...
					<tr>
						...
						<td id="banner">
							<?php 
								if(!empty($this->request->data['ProductMaker']['banner'])){
									if(!empty($this->request->data['ProductMaker']['banner'])) $img = '/timthumb.php?src='.$this->Html->url('/webroot/img/images/product_makers/').$this->request->data['ProductMaker']['banner'];
									$img_small = $img."&h=60&w=200&zc=2";
									$img_larger = $img."&h=150&w=500&zc=2";
									
									echo $this->Html->link($this->Html->image($img_small,array('alt'=>$this->request->data['ProductMaker']['name'])),$img_larger,array('title'=>$this->request->data['ProductMaker']['name'],'class'=>'preview','target'=>'_blank','escape'=>false));
									echo $this->Html->link('&nbsp;',"javascript:delImg({$this->request->data['ProductMaker']['id']},'banner');",array('title'=>'Xóa ảnh','class'=>'act delete tooltip ask','escape'=>false)).'<br />';
								}

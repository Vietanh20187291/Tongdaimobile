...
<div id="column_right">
	<div id="action_top">
		...
		<?php echo $this->Form->create('Video',array('type'=>'get','name'=>'search','url'=>array('controller'=>'videos','action'=>'index'),'inputDefaults'=>array('div'=>false,'label'=>false)))?>
		<ul class="search">
	...
	<?php 
		echo $this->Form->create('VideoCategory',array('type'=>'post','name'=>'form','inputDefaults'=>array('div'=>false,'label'=>false)));
	...
	<div id="content">
		<table class="list">
			...
			<?php foreach($a_categories_c as $val){
				$item_category = $val['VideoCategory'];
				
				$url_view = array('controller'=>'videos','action'=>'index','lang'=>$item_category['lang'],'slug0'=>$item_category['slug'],'admin'=>false);
				$url_edit = array('controller'=>'video_categories','action'=>'edit',$item_category['id'],'?'=>array('url'=>$current_url_c));
			?>
			<tr id="<?php echo 'item_'.$item_category['id']?>">
				<td class="center"><input type="checkbox" name="chkid[]" value="<?php echo $item_category['id']?>" /></td>
				<td>
					<?php 
						echo $this->Html->link($this->Text->truncate($item_category['name'],200,array('extact'=>false)),array('controller'=>'videos','action'=>'index','?'=>array('cate_id'=>$item_category['id'])),array('title'=>$item_category['name'],'class'=>'tooltip'));
						$a_counter = unserialize($item_category['counter']);
						echo ' <span class="counter">[ '.$this->Html->link(number_format($a_counter['item']),array('controller'=>'videos','action'=>'index','?'=>array('cate_id'=>$item_category['id'])),array('title'=>number_format($a_counter['item']).' '.__('video',true),'class'=>'tooltip'))
								 .' ]</span>';
					?>
				</td>

...
<div id="column_right">
	...
	<div id="content">
		<?php 
			echo $this->Form->create('Document',array('type'=>'file','id'=>'form','inputDefaults'=>array('label'=>false,'div'=>false)));
			...
			<div id="tab1" class="tab_content">
				<table class="add">
					...
					<tr>
						...
						<td>
							<?php 
								echo $this->Form->input('document_category_id',array('type'=>'select','options'=>$a_list_categories_c,'empty'=>__('Chọn danh mục',true),'class'=>'medium','required'=>true));
								...
								echo '('.count($this->request->data['Document']['category_other']).')';
					...
					<tr>
						...
						<td>
							<?php
								$value_type = 'file';
								if(!empty($this->request->data['Document']['link'])) $value_type = 'link'; 
								echo $this->Form->input('type',array('type'=>'select','options'=>array('file'=>__('Upload lên server',true),'link'=>__('Link từ bên ngoài',true)),'value'=>$value_type,'class'=>'medium'));
							?>
						</td>
					</tr>
					<tr id="d_link">
						...
						<td>
							<?php 
								$value_link = $this->request->data['Document']['link'];
								if(empty($value_link)) $value_link = 'http://';
								echo $this->Form->input('link',array('class'=>'larger','value'=>$value_link));
								if(!empty($a_errors_c['link'])) echo $this->Form->label('link',__('This field is required.',true),array('class'=>'error'));
							?>
						</td>
					</tr>
					<tr id="d_file">
						<th><?php echo $this->Form->label('file',__('Tệp tin',true))?></th>
						<td>
							<?php 
								if(!empty($this->request->data['Document']['file'])) echo $this->Html->link($this->request->data['Document']['file'],'/img/files/documents/'.$this->request->data['Document']['file'],array('title'=>$this->request->data['Document']['file'],'target'=>'_blank')).'<br>';

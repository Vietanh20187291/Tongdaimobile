<div id="support_2">
	...
	<a href="javascript:;" class="title"><?php echo __('Hỗ trợ trực tuyến',true)?></a>
	<div>
		<ul>
			...
			<li>
				...
				<p class="nick yahoo">
					<a title="<?php echo __('Chat yahoo với',true).' '.$item_support['name'].' ('.$item_support['yahoo'].')'?>" href="ymsgr:sendIM?<?php echo $item_support['yahoo']?>" rel="nofollow">
						<img alt="Yahoo" src="http://opi.yahoo.com/online?u=<?php echo $item_support['yahoo']?>&amp;m=g&amp;img/&amp;l=us">
					</a>
				</p>
				<?php } if(!empty($item_support['skype'])){?>
				<p class="nick skype">
					<a href="skype:<?php echo $item_support['skype'] ?>?chat" onclick="return skypeCheck();" title="<?php echo __('Chat skype với',true).' '.$item_support['name'].' ('.$item_support['skype'].')'?>" rel="nofollow"><?php echo $this->Html->image('skype64.png',array('skype icon'))?></a>
				</p>
				<?php }?>
			</li>
			<?php }?>

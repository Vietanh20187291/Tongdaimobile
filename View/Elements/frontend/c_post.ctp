...
<div class="<?php if(!empty($class)) echo $class;?> c_post">
	...
	<div class="thumb col-lg-3 col-xs-12 col-sm-3 col-md-3">
		<?php if ($item_post['user_id'] != 14){

		 	echo $this->Html->link($this->OnewebVn->thumb('posts/'.$item_post['image'],array('alt'=>$item_post['meta_title'],'width'=>$w,'height'=>$h, 'zc' => '1', 'class'=>'img-responsive')),$url,$link_img_attr);


        		 }else{
			echo $this->Html->link($this->Html->image($item_post['image'],array('alt'=>$item_post['meta_title'],'width'=>$w,'height'=>$h, 'zc' => '1', 'class'=>'img-responsive')),$url,$link_img_attr);
	...
	<div class="col-lg-9 col-xs-12 col-sm-9 col-md-9">
		...
		if ($item_post['user_id'] != 14) {
			...
			echo $this->Html->tag('p', $this->Text->truncate(trim(strip_tags($item_post['meta_description'])), array('exact' => false)), array('class' => 'sumary'));
		}

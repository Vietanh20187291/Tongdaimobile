...
<aside class="box post">
	...
	<div class="nav-v-post" id="tree">
		...
		<?php foreach ($a_post_categories_s as $val){
			...
			$item_post = $val['ChildPostCategory'];
			if(empty($item_cate['link'])){
				$url = array('controller'=>'posts','action' => 'index','lang'=>$item_cate['lang'],'position'=>$item_cate['position']);
			...
			<li>
				...
				?>
					<ul>
						...
						$url = array('controller'=>'posts','action'=>'index','lang'=>$val1['lang'],'position'=>$item_cate['position']);

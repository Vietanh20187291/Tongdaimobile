...
<div id="column_right">
	...
	<div id="content">
		<?php  
			echo $this->Form->create('PostCategory',array('type'=>'file','id'=>'form','url'=>array('action'=>'edit','?'=>array('url'=>(!empty($_GET['url']))?urldecode($_GET['url']):'')),'inputDefaults'=>array('label'=>false,'div'=>false)));
			...
			<div id="tab1" class="tab_content">
				<table class="add">
					<tr>
						...
						<td><?php echo $this->Form->input('name',array('class'=>'larger','onchange'=>'getFieldByName("PostCategory")'))?></td>
					</tr>
					<?php if(!empty($this->request->data['PostCategory']['parent_id'])){?>
					<tr>
						<th><?php echo $this->Form->label('parent_id',__('Danh mục cha',true))?> <span class="im">*</span></th>
					...
					<tr <?php if(empty($oneweb_post['category_image'])) echo ' class="hidden"'?>>
						<th><?php echo $this->Form->label('image',__('Ảnh đại diện',true))?></th>
						<td id="image">
							<?php 
								if(!empty($this->request->data['PostCategory']['image'])){
									if(!empty($this->request->data['PostCategory']['image'])) $img = '/timthumb.php?src='.$this->Html->url('/webroot/img/images/post_categories/').$this->request->data['PostCategory']['image'];
									$img_small = $img."&h=90&w=90&zc=2";
									$img_larger = $img."&h=300&w=300&zc=2";
									
									echo $this->Html->link($this->Html->image($img_small,array('alt'=>$this->request->data['PostCategory']['name'])),$img_larger,array('title'=>$this->request->data['PostCategory']['name'],'class'=>'preview','target'=>'_blank','escape'=>false));
									echo $this->Html->link('&nbsp;',"javascript:delImg({$this->request->data['PostCategory']['id']},'image');",array('title'=>'Xóa ảnh','class'=>'act delete tooltip ask','escape'=>false)).'<br />';
								}
								echo $this->Form->input('image',array('type'=>'file'));
								echo $this->Html->tag('span',__('Kích thước',true).": {$oneweb_post['size']['category'][0]} x {$oneweb_post['size']['category'][1]} (px)",array('class'=>'size_img'));
							?>
						</td>
					</tr>
					<tr <?php if(empty($oneweb_post['category_banner'])) echo ' class="hidden"'?>>
						<th><?php echo $this->Form->label('banner',__('Banner',true))?></th>
						<td id="banner">
							<?php 
								if(!empty($this->request->data['PostCategory']['banner'])){
									if(!empty($this->request->data['PostCategory']['banner'])) $img = '/timthumb.php?src='.$this->Html->url('/webroot/img/images/post_categories/').$this->request->data['PostCategory']['banner'];
									$img_small = $img."&h=60&w=200&zc=2";
									$img_larger = $img."&h=150&w=500&zc=2";
									
									echo $this->Html->link($this->Html->image($img_small,array('alt'=>$this->request->data['PostCategory']['name'])),$img_larger,array('title'=>$this->request->data['PostCategory']['name'],'class'=>'preview','target'=>'_blank','escape'=>false));
									echo $this->Html->link('&nbsp;',"javascript:delImg({$this->request->data['PostCategory']['id']},'banner');",array('title'=>'Xóa ảnh','class'=>'act delete tooltip ask','escape'=>false)).'<br />';
								}
								echo $this->Form->input('banner',array('type'=>'file'));
								echo $this->Html->tag('span',"Kích thước: {$oneweb_post['size']['category_banner'][0]} x {$oneweb_post['size']['category_banner'][1]} (px)",array('class'=>'size_img'));
							?>
						</td>
					</tr>
					<tr<?php if(empty($oneweb_post['category_banner'])) echo ' class="hidden"'?>>
							...
							<div class="tab_container_2">
								<div id="tab21" class="tab_content_2">
									<?php 
										...
										echo $this->CkEditor->create('PostCategory.description',array('toolbar'=>'full'));	
									?>
								</div> <!-- end #tab21 -->

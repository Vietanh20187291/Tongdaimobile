...
<div id="column_right">
	<div id="action_top">
		...
		<?php echo $this->Form->create('Document',array('type'=>'get','name'=>'search','url'=>array('controller'=>'documents','action'=>'index'),'inputDefaults'=>array('div'=>false,'label'=>false)))?>
		<ul class="search">
	...
	<?php 
		echo $this->Form->create('DocumentCategory',array('type'=>'post','name'=>'form','inputDefaults'=>array('div'=>false,'label'=>false)));
	...
	<div id="content">
		<table class="list">
			...
			<?php foreach($a_categories_c as $val){
				...
				$url_view = array('controller'=>'documents','action'=>'view','lang'=>$item_category['lang'],'slug_cate'=>$item_category['slug'],'admin'=>false);
				$url_edit = array('controller'=>'document_categories','action'=>'edit',$item_category['id'],'?'=>array('url'=>$current_url_c));
			?>
			<tr id="<?php echo 'item_'.$item_category['id']?>">
				<td class="center"><input type="checkbox" name="chkid[]" value="<?php echo $item_category['id']?>" /></td>
				<td>
					<?php 
						echo $this->Html->link($this->Text->truncate($item_category['name'],200,array('extact'=>false)),array('controller'=>'documents','action'=>'index','?'=>array('category_id'=>$item_category['id'])),array('title'=>$item_category['name'],'class'=>'tooltip'));
						
						$a_counter = unserialize($item_category['counter']);
						echo ' <span class="counter">[ '.$this->Html->link(number_format($a_counter['item']),array('controller'=>'documents','action'=>'index','?'=>array('category_id'=>$item_category['id'])),array('title'=>number_format($a_counter['item']).' '.__('album',true),'class'=>'tooltip'))
								 .' ]</span>';
					?>
				</td>
				...
				<td class="center status"><?php echo $this->Html->link('&nbsp;','javascript:;',array('title'=>'Thay đổi','onclick'=>"changeStatus({$item_category['id']})",'escape'=>false,'class'=>'act tooltip '.(($item_category['status']==1)?'active':'unactive')));?></td>

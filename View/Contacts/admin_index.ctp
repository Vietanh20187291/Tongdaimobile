...
<div id="column_right">
	<div id="action_top">
		...
		<?php echo $this->Form->create('Contact',array('url'=>array('action'=>'index'),'type'=>'get','name'=>'search','inputDefaults'=>array('div'=>false,'label'=>false)))?>
		<ul class="search">
	...
	<?php 
		echo $this->Form->create('Contact',array('type'=>'contact','name'=>'form','inputDefaults'=>array('div'=>false,'label'=>false)));
	...
	<div id="content">
		<table class="list">
			<tr class="first">
				...
				<th><?php echo $this->Paginator->sort('subject',__('Tiêu đề',true))?></th>
				<th class="small"><?php echo $this->Paginator->sort('name',__('Họ tên',true))?></th>
				<th class="small center"><?php echo $this->Paginator->sort('ContactCategory.name',__('Nhóm',true))?></th>
			...
			<?php foreach($a_contacts_c as $val){
				...
				$url_view = array('controller'=>'contacts','action'=>'view',$item_contact['id'],'?'=>array('url'=>$current_url_c));
			?>
			<tr id="<?php echo 'item_'.$item_contact['id']?>">
				<td class="center"><input type="checkbox" name="chkid[]" value="<?php echo $item_contact['id']?>" /></td>
				<td<?php if(!$item_contact['view']) echo ' class="new"'?>><?php echo $this->Html->link($this->Text->truncate($item_contact['subject'],50),$url_view,array('title'=>$item_contact['subject'],'class'=>'tooltip'))?></td>
				<td class="small">
					<?php 
						...
						.$this->Html->link($item_contact['phone'],array('action'=>'index','?'=>array('keyword'=>$item_contact['phone'])),array('title'=>$item_contact['phone']))
						,array('class'=>'email'))
					?>
				</td>
				<td class="small"><?php echo $this->Form->input('contact_category_id',array('type'=>'select','options'=>$a_list_categories_c,'class'=>'medium','empty'=>'Chọn nhóm','value'=>$item_category['id'],'onchange'=>"setCategory({$item_contact['id']},this.value)"))?></td>
				<td class="center">
					<?php 
